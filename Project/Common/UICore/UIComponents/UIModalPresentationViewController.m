//
//  UIModalPresentationViewController.m
//  Project
//
...
@property(nonatomic, weak, readwrite) UIModalPresentationViewController *modalPresentedViewController;
@end

@implementation UIModalPresentationViewController (UIAppearance)
static UIModalPresentationViewController *appearance;
...
+ (void)initDefaultAppearance {
  ...
  appearance.animationStyle = UIModalPresentationAnimationStyleFade;
  appearance.contentViewMargins = UIEdgeInsetsMake(20, 20, 20, 20);
  appearance.maximumContentViewWidth = ([UIHelper screenSizeFor47Inch].width - UIEdgeInsetsGetHorizontalValue(appearance.contentViewMargins));
}
...
@interface UIModalPresentationViewController ()
@property(nonatomic, strong) UIModalPresentationWindow *containerWindow;
...
@implementation UIModalPresentationViewController
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  [UIHelper dimmedApplicationWindow];
...
- (void)viewWillDisappear:(BOOL)animated {
  ...
  [UIHelper resetDimmedApplicationWindow];
...
- (void)setContentViewController:(UIViewController<UIModalPresentationContentViewControllerProtocol> *)contentViewController {
...
- (void)showingAnimationWithCompletion:(void (^)(BOOL))completion {
  if (self.animationStyle == UIModalPresentationAnimationStyleFade) {
    self.dimmingView.alpha = 0.0;
    self.contentView.alpha = 0.0;
    [UIView animateWithDuration:.2 delay:0.0 options:UIViewAnimationOptionsCurveOut animations:^{
  ...
  } else if (self.animationStyle == UIModalPresentationAnimationStylePopup) {
    self.dimmingView.alpha = 0.0;
    self.contentView.transform = CGAffineTransformMakeScale(0, 0);
    [UIView animateWithDuration:.3 delay:0.0 options:UIViewAnimationOptionsCurveOut animations:^{
      self.dimmingView.alpha = 1.0;
  ...
  } else if (self.animationStyle == UIModalPresentationAnimationStyleSlide) {
    self.dimmingView.alpha = 0.0;
...
- (void)showWithAnimated:(BOOL)animated completion:(void (^)(BOOL))completion {
  ...
  if (!self.containerWindow) {
    self.containerWindow = [[UIModalPresentationWindow alloc] init];
    self.containerWindow.windowLevel = UIWindowLevelUIAlertView;
    self.containerWindow.backgroundColor = UIColorClear;// 避免横竖屏旋转时出现黑色
  }
  self.supportedOrientationMask = [UIHelper visibleViewController].supportedInterfaceOrientations;
...
- (void)handleKeyboardWillShow:(NSNotification *)notification {
  CGFloat keyboardHeight = [UIHelper keyboardHeightWithNotification:notification inView:self.view];
  if (keyboardHeight > 0) {
...
- (BOOL)shouldAutorotate {
  UIViewController *visibleViewController = [UIHelper visibleViewController];
...
@implementation UIModalPresentationViewController (Manager)

+ (BOOL)isAnyModalPresentationViewControllerVisible {
  for (UIWindow *window in [[UIApplication sharedApplication] windows]) {
    if ([window isKindOfClass:[UIModalPresentationWindow class]] && !window.hidden) {
      return YES;
    }
  }
  return NO;
...
+ (BOOL)hideAllVisibleModalPresentationViewControllerIfCan {
  ...
  for (UIWindow *window in [[UIApplication sharedApplication] windows]) {
    if (![window isKindOfClass:[UIModalPresentationWindow class]]) {
      continue;
    }
    ...
    UIModalPresentationViewController *modalViewController = (UIModalPresentationViewController *)window.rootViewController;
    BOOL canHide = YES;
...
@implementation UIModalPresentationWindow

@end

@implementation UIViewController (UIModalPresentationViewController)

static char kAssociatedObjectKey_ModalPresentationViewController;
- (void)setModalPresentedViewController:(UIModalPresentationViewController *)modalPresentedViewController {
...
- (UIModalPresentationViewController *)modalPresentedViewController {
  return (UIModalPresentationViewController *)objc_getAssociatedObject(self, &kAssociatedObjectKey_ModalPresentationViewController);
}
