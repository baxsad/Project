...
- (void)didInitializedWithStyle:(UITableViewCellStyle)style {
  _cellPosition = UITableCellScenePositionNone;
  _style = style;
  _enabled = YES;
...
- (void)initDefaultAccessoryButtonIfNeeded {
  if (!self.defaultAccessoryButton) {
    self.defaultAccessoryButton = [[UIButton alloc] init];
...
- (void)handleAccessoryButtonEvent:(UIButton *)detailButton {
  if ([self.parentTableView.delegate respondsToSelector:@selector(tableView:accessoryButtonTappedForRowWithIndexPath:)]) {
    [self.parentTableView.delegate tableView:self.parentTableView accessoryButtonTappedForRowWithIndexPath:[self.parentTableView indexPathForRowAtView:detailButton]];
  }
}
...
- (void)updateCellAppearanceWithIndexPath:(NSIndexPath *)indexPath {
  ...
  if (indexPath && self.parentTableView) {
    UITableCellScenePosition position = [self.parentTableView positionForRowAtIndexPath:indexPath];
