//
//  UIStaticTableViewCellDataSource.h
//  StarShareDevUIKit
//
...
@class UIStaticTableViewCellData;
 ...
 *  3. 将第 2 步里的 dataSource 赋值给 tableView.staticCellDataSource 即可完成一般情况下的界面展示。
 *  4. 若需要重写某些 UITableViewDataSource、UITableViewDelegate 方法，则在 viewController 里直接实现该方法，并在方法里调用 UIStaticTableViewCellDataSource (Manual) 提供的同名方法即可。
 */
@interface UIStaticTableViewCellDataSource : NSObject

/// 列表的数据源，是一个二维数组，其中一维表示 section，二维表示某个 section 里的 rows，每次调用这个属性的 setter 方法都会自动刷新 tableView 内容。
@property(nonatomic, copy) NSArray<NSArray<UIStaticTableViewCellData *> *> *cellDataSections;

/// 数据源绑定到的列表，在 UITableView (QMUI_StaticCell) 里会被赋值
@property(nonatomic, weak, readonly) UITableView *tableView;

- (instancetype)init NS_DESIGNATED_INITIALIZER;
- (instancetype)initWithCellDataSections:(NSArray<NSArray<UIStaticTableViewCellData *> *> *)cellDataSections NS_DESIGNATED_INITIALIZER;

@end

/// 当需要重写某些 UITableViewDataSource、UITableViewDelegate 方法时，这个分类里提供的同名方法需要在该方法中被调用，否则可能导致 UIStaticTableViewCellData 里设置的一些值无效。
@interface UIStaticTableViewCellDataSource (Manual)

/**
 *  从 dataSource 里获取处于 indexPath 位置的 UIStaticTableViewCellData 对象
 ...
 */
- (__kindof SSUITableCell *)cellForRowAtIndexPath:(NSIndexPath *)indexPath;
